...
# ── Глобальный КД на публикацию ──────────────────────────────────────────

_last_post_time: datetime | None = None  # глобальный КД в памяти
...
def get_cooldown_remaining() -> int:
    ...
    return max(0, int(TASK_COOLDOWN_SECONDS - elapsed))

def set_cooldown():
    """Фиксирует время последнего поста (общее для всех)."""
...
async def start_task_creation(query, context):
    ...
    if remaining > 0:
        ...
        await query.answer()
        await query.edit_message_text(
            f"⏳ <b>Кулдаун активен</b>\n\nНедавно был выставлен набор.\nСледующий можно выставить через: <b>{mins} мин {secs} сек</b>\nОткроется в: <b>{unlock_time}</b>",
            parse_mode="HTML",
...
async def handle_close_task(query, context):
    ...
    try:
        ...
        try:
            ...
            pass  # пост мог быть удалён из канала

        # Помечаем закрытым (не удаляем — cant_write должен найти admin_id)
        save_tasks(tasks)
...
async def show_platform_management(query, context):
    ...
    for i, name in enumerate(platforms.keys(), 1):
        keyboard.append([InlineKeyboardButton(f"❌ {name}", callback_data=f"delete_platform_{i}")])
    keyboard.append([InlineKeyboardButton("Назад", callback_data="back_to_main")])
    try:
...
def main():
    ...
    # Обработчик кнопки "Не могу написать" — работает всегда, вне бота
